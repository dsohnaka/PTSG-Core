...
Two Ties deferred from Chapter 3 v1.1 are prescaler-coupled and are addressed here. **In v1.1, C3-T11 (Stay Set role) is RESOLVED to Fixed (C4-F10) on the strength of silicon evidence; C3-T10 (prescale edge) is RESOLVED to Fixed (C4-F11, trailing edge) by the Trailing-Edge Doctrine (Chapter 1 § 1.4a).**

第3章 v1.1 から繰り延べられた二つの Tie がプリスケーラ結合であり、ここで扱われる。**v1.1 において、C3-T11（Stay Set 役割）は実機エビデンスに基づき Fixed（C4-F10）へ解決される；C3-T10（プリスケール縁）は後縁主義（第1章 § 1.4a）により Fixed（C4-F11、後縁）へ解決される。**
...
**Resolution (v1.1) — RESOLVED to the trailing edge, now Fixed C4-F11.** Under the Trailing-Edge Doctrine (Chapter 1 § 1.4a), a queued operation fires **at the trailing edge** — the moment its count completes — so the Stay literally ends when it should and the next state's leading edge begins into a settled world. The earlier hybrid lean (leading-edge match flag) is **superseded**: emitting a defining action on the leading edge contradicts the doctrine that the leading edge is already-settled. A sustained external strobe, where an application needs one, is a Formation-side concern *derived from* the trailing-edge match pulse, not a Core leading-edge action. Promoted from Tie C4-T3 to **Fixed C4-F11**.
...
**解決（v1.1）—— 後縁へ解決、Fixed C4-F11 へ。** 後縁主義（第1章 § 1.4a）の下、キュー演算は**後縁で**——そのカウントが完了する瞬間に——発火し、ゆえに Stay は終わるべき時に文字通り終わり、次状態の前縁は静定した世界で始まる。以前のハイブリッド傾向（前縁一致フラグ）は**置き換えられる**: 前縁で定義的アクションを発することは、前縁が既に静定しているという原則に反する。応用が持続的な外部ストローブを要する場合、それは後縁一致パルス*から導かれる* Formation 側の関心事であって、Core の前縁アクションではない。Tie C4-T3 から **Fixed C4-F11** へ昇格。
...
| **C4-F10** (v1.1; wording corrected 2026-07) | **Stay Set role = clear/sync-only** (was Tie C4-T4 / C3-T11). Stay Set resets/arms the stay counter, **which counts prescaler ticks from Stay Set onward (On-Tick), through the window; the Stay instruction never clears it** (RH003/004/005). Stay-timeup = the Nth tick after Stay Set on the free-running grid — independent of the background-program's clock-length (jitter-free). Silicon-confirmed via duty idiom D. *(An earlier v1.1 text mis-stated "the count begins at Prog End"; corrected per the as-built RTL — see Ch3 § 3.2 and Layer 2 trace 2026-07-06_ptsg-command-phase-table.)* / **Stay Set 役割 = クリア／同期のみ**（旧 Tie C4-T4 / C3-T11）。Stay Set はステイカウンタをリセット/アームし、**カウンタは Stay Set 以降プリスケーラティックを数え（On-Tick）、窓を通して継続;Stay 命令は決してクリアしない**（RH003/004/005）。Stay-timeup = 自由走行グリッド上の Stay Set 後第 N tick——背景プログラムのクロック長から独立（ジッタ無縁）。流儀 D で実機確認済み。*（以前の v1.1 文は「カウントは Prog End で開始」と誤記;as-built RTL に従い訂正——第3章 § 3.2 と Layer 2 トレース参照。）* | **F** (v1.1, was Tie C4-T4) |
...
| **C4-F11** (v1.1, was Tie C4-T3) | **Prescale edge for queued execution = trailing edge.** A queued operation fires at the trailing edge of the prescale period (the moment its count completes), per the Trailing-Edge Doctrine (Chapter 1 § 1.4a). The earlier leading-edge-flag hybrid is superseded; a defining action on the leading edge would contradict the doctrine. Sustained external strobes are a Formation concern derived from the trailing-edge match pulse. (Distinct from the prescaler *phase* question, which C4-F9 resolves.) / **キュー実行のプリスケール縁 = 後縁。** キュー演算はプリスケール周期の後縁（カウント完了の瞬間）で発火する、後縁主義（第1章 § 1.4a）に従い。以前の前縁フラグ・ハイブリッドは置き換えられる；前縁の定義的アクションは原則に反する。持続的外部ストローブは後縁一致パルスから導かれる Formation の関心事。（プリスケーラ*位相*の問いとは別個で、それは C4-F9 が解決。） | **F** (v1.1) |

**Decision count by status (v1.1):** Fixed (F): 11; Convention (V): 3; Tie (T): 2.

**地位別決定数（v1.1）:** Fixed (F): 11；Convention (V): 3；Tie (T): 2.

Notable (v1.1): the two Ties inherited from Chapter 3 v1.1 (formerly C3-T10, C3-T11) are **both now resolved to Fixed**: C3-T11 (Stay Set role) → **C4-F10** on silicon evidence; C3-T10 (prescale edge) → **C4-F11** (trailing edge) by the Trailing-Edge Doctrine (Chapter 1 § 1.4a). Four new v1.1 Fixed items (C4-F8 foreground prescaling, C4-F9 free-running phase-lock, C4-F10 Stay Set role, C4-F11 trailing-edge firing) and one Convention (C4-V3 state-0) record the verification campaign and the doctrine.

注目すべき（v1.1）: 第3章 v1.1 から継承された二つの Tie（旧 C3-T10、C3-T11）は**いずれも Fixed へ解決**された: C3-T11（Stay Set 役割）→ 実機エビデンスで **C4-F10**；C3-T10（プリスケール縁）→ 後縁主義（第1章 § 1.4a）により **C4-F11**（後縁）。四つの新規 v1.1 Fixed 項目（C4-F8 前景プリスケールド化、C4-F9 自由走行位相ロック、C4-F10 Stay Set 役割、C4-F11 後縁発火）と一つの Convention（C4-V3 state-0）が検証キャンペーンと原則を記録する。
