...
iverilog -g2012 -o simc ptsg_core.v ptsg_core_conformance_tb.v \
    ../ai_friendly_vendor_wrappers/ptsg_imem/ptsg_imem.v
vvp simc
# Expected: PASS T1..T18, ALL CONFORMANCE TESTS PASSED
```
...
- **Queued band (after Prog End)** implements a single queued-operation slot
  ...
  §3.4b's Branch Q row), and **Call** (Phase 3b — unconditional, plain
  auto-save + jump at Stay-timeup). **Reset** is queued separately (its own
  independent reservation, not sharing this slot) and wins with absolute
  priority at Stay-timeup over whatever the slot holds. **Return** still
  executes immediately regardless of band (documented deviation; queuing it
  is Phase 3c work). Overwrite arbitration beyond simple last-write-wins
  (C8's SN-reservation-overwrite HALT) needs the Phase-4 S_HALT machinery and
